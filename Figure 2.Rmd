...
p = kerenKontext |> 
  ...
  geom_point(size = 4) +
  gghighlight::gghighlight(test %in% c("Keratin+ Tumour__CD8__immune", "DC/Mono__Macrophages__immune"),
...
#ggsave("figures/keren/Image5Immune.pdf", device = "pdf", dpi = 400, height = 5, width = 6)

kerenSCE5 = kerenSCE |> 
  filter(imageID == 5)

kerenInhom5 = Statial::Kontextual(
  kerenSCE5, 
  parentDf = parentDf,
  parent = immune,
  inhom = TRUE,
...
kerenInhom5 |> 
  filter(test %in% c("DC/Mono__Macrophages__immune", "Keratin+ Tumour__CD8__immune"))
```
...
image21Immune = kerenSCE |> 
  ...
  ggplot(aes(x = x, y = y)) +
  geom_point(col = "#ff891c", size = 2.3) +
  theme_void() +
  theme(legend.position = "none") +
  geom_segment(aes(y = -100, yend = -100,
                   xend = max(kerenSCE6Plot$x), x = max(kerenSCE6Plot$x) - 100*scale),
...
n <- 100  # Number of points to generate (more points for smoother density)
...
cornerDf = data.frame(
  ...
  cellType = "B") |>
  filter( x^2 + y^2 > (radius + 3)^2) #|> 
  #filter(!(x > 0  & y > 0))

kontextCorner = data.frame(
  x = runif(100, min = 0, max = 6),
  y = runif(100, min = 0, max = 6),
  cellType = "B") |> 
  filter( x^2 + y^2 > (radius - 0.5)^2)  |> 
  filter( x^2 + y^2 < (radius + 0.5)^2) 
  
thinDf = rbind(circleDf, cornerDf, kontextCorner) |> 
  mutate(imageID = 1)
...
thinPlot = thinDf |> 
  ...
  geom_point() +
  theme_void() +
  scale_color_manual(values=c("#E25758", "#404040",  "#D7D8D8")) +
  theme(legend.position = "none")
...
#ggsave("figures/simulation/thinPlot.pdf", plot = thinPlot, device = "pdf", dpi = 400, height = 5, width = 6)
```
...
#1280 is the first scale with a disnfiicant
crawdadScore = dat |> 
...
crawdadPerm = bplapply(kerenSCEShuffled[1:50], function(sce) {
  ...
  sceSF <- crawdad:::toSF(pos = sceDf[, c("x", "y")],
                          cellTypes = sceDf[, "cellType"])
  
  ## shuffle cells to create null background
  shuffle_list <- crawdad:::makeShuffledCells(sceSF,
                                            scales = 500*scale,
                                            perms = 1,
                                            ncores = 1,
                                            seed = 101,
  ...
  results <- crawdad::findTrends(sceSF,
                               neighDist = 50*scale,
                               shuffleList = shuffle_list,
                               ncores = 1,
                               verbose = TRUE,
...
methodCompareRes$permutedZscore = c(
  ...
  (crawdadScore - mean(crawdadPerm))/sd(crawdadPerm)
