...
#lazyload_cache_labels(labels = chunk, path = "MSFigures_cache/html")

#path = "/albona/nobackup2/"
...
plotKontextScatter = function(kontextResult, r, highlightVal = "test") {
  ...
  summarise(original = mean(original, na.rm = TRUE)/sd(original, na.rm = TRUE),
            kontextual = mean(kontextual, na.rm = TRUE)/sd(kontextual, na.rm = TRUE)) |>
...
coxTests <- function(measurementMat, outcome, remove = c(NA)) {
  result <- lapply(colnames(measurementMat), function(test) {
    ...
    fit <- coxph(surv ~ measurementCol, subset = ind)
  ...
  result <- result |>
    dplyr::bind_rows() |>
    dplyr::mutate(test = names(measurementMat)) |>
...
xeniumBreastKontext100 |> 
  separate(test, sep = "__", into = c("a", "b", "parent")) |> 
  filter(a %in% c("CD8 T", malignant)) |> 
  filter(b %in% c("CD8 T", malignant))
